...
the Python 3 class initialization, S+1 error analysis, root bracketing, state
restoration, input validation, and several non-terminating-loop edge cases.
...
def errorAnalysis(
    ...
    """Perform the original one-parameter-at-a-time S+1 analysis.

    Other parameters remain fixed at their optimum while one parameter is
    varied. The returned entries are [best value, lower error, upper error].
    """
    ...
    print(f"# PERFORM S+1 ERROR ANALYSIS; S_min = {best_quality:.12g}")
    ...
    try:
        for parameter_id, optimize in enumerate(opt_flags):
            ...
            def objective(
                value: float, parameter_id: int = parameter_id
            ) -> float:
                trial = best.copy()
                trial[parameter_id] = value
                return function_object.scaleData(trial) - (best_quality + 1.0)
...
OPTIONS
    ...
    -getError          Calculate one-parameter S+1 error intervals.
...
EXAMPLE
    python {program_name} -f inputFiles.dat -xc 0.592541 \\
        -a 0.754524 -b 0.107421 -xr -1 1 -getError -o error.out
""".strip()
    )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        ...
        if get_error:
            ...
            output.write("# S+1 error analysis yields:\n")
